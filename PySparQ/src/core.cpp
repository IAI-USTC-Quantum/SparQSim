...
PYBIND11_MODULE(_core, m)
{
	...
	m.def("qda_solve", &QDA_solve,
		  "A"_a, "b"_a,
		  "kappa"_a = py::none(),
		  "p"_a = 1.3,
		  "step_rate"_a = 0.01);

	m.def("qda_classical2quantum", &QDA_classical2quantum, "A"_a, "b"_a);

	py::class_<DenseMatrix<complex_t>>(m, "DenseMatrix_complex128")
		.def(py::init<Eigen::MatrixXcd>(), "matrix"_a);

	py::class_<DenseMatrix<double>>(m, "DenseMatrix_float64")
		.def(py::init<Eigen::MatrixXd>(), "matrix"_a);

	py::class_<DenseVector<complex_t>>(m, "DenseVector_complex128")
		.def(py::init<Eigen::VectorXcd>(), "vector"_a);

	py::class_<DenseVector<double>>(m, "DenseVector_float64")
		.def(py::init<Eigen::VectorXd>(), "vector"_a);
	...
	py::class_<System>(m, "System")
		...
		.def_readonly_static("max_qubit_count", &System::max_qubit_count)
	...
	sparse_state.def_readonly("basis_states", &SparseState::basis_states)
		...
		.def("__str__", [](const SparseState& self) { return self.to_string(0); })
	...
	BIND_BASE_OPERATOR_SUBNAME(CondRot_General_Bool_Cpp, "CondRot_General_Bool")
	...
	py::class_<PartialTraceSelect>(m, "PartialTraceSelect")
		...
		.def("get_projected_full",
			 (std::pair<std::vector<complex_t>, double> (PartialTraceSelect::*)(SparseState &) const) & PartialTraceSelect::get_projected_full,
			 py::arg("state"));
	...
	py::class_<qram_qutrit::QRAMCircuit>(m, "QRAMCircuit_qutrit")
		...
		.def(py::init([](size_t addr_size, size_t data_size, const Eigen::Matrix<uint64_t, Eigen::Dynamic, 1> &eigen_vec)
					  {
...

	{
		...
		BIND_BASE_OPERATOR(State_Prep_via_QRAM)
	...
	}
	{
		...
		// py::class_<
		// 	Walk_s_via_QRAM,
		// 	Walk_s<Block_Encoding_via_QRAM, State_Prep_via_QRAM>>(m, "Walk_s_via_QRAM")
		py::class_<Walk_s_via_QRAM>(m, "Walk_s_via_QRAM")
			.def(py::init<qram_simulator::qram_qutrit::QRAMCircuit *,
						  qram_qutrit::QRAMCircuit *,
						  std::string_view,
						  std::string_view,
						  std::string_view,
						  std::string_view,
				 ...
				 "data_size"_a,
				 "rational_size"_a)
			.def("__call__", [](const Walk_s_via_QRAM &self, SparseState &state)
				 { self.operator()(state); })
			.def("dag", [](const Walk_s_via_QRAM &self, SparseState &state)
				 { self.dag(state); });
	}
	...
	BIND_BASE_OPERATOR(Rot_Bool)
		.def(py::init([](std::string_view reg_, size_t digit_, const std::array<std::complex<double>, 4> &mat)
					  { return new Rot_Bool(reg_, digit_, u22_t(mat)); }),
			 py::arg("reg"), py::arg("digit"), py::arg("matrix"))
		.def(py::init([](size_t id_, size_t digit_, const std::array<std::complex<double>, 4> &mat)
					  { return new Rot_Bool(id_, digit_, u22_t(mat)); }),
			 py::arg("reg_id"), py::arg("digit"), py::arg("matrix"))
		.def(py::init([](std::string_view reg_, const std::array<std::complex<double>, 4> &mat)
					  { return new Rot_Bool(reg_, u22_t(mat)); }),
			 py::arg("reg"), py::arg("matrix"))
		.def(py::init([](size_t id_, const std::array<std::complex<double>, 4> &mat)
					  { return new Rot_Bool(id_, u22_t(mat)); }),
			 py::arg("reg_id"), py::arg("matrix"))
			BIND_CONTROLLABLE_METHODS(Rot_Bool);
