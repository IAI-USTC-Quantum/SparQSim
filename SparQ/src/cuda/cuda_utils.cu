/**
 * @file cuda_utils.cu
 * @brief CUDA parallel implementation of cuda_utils
 * @details Implements with thrust device vectors and CUDA kernels the GPU helper functions declared in cuda_utils.cuh
 *          (device properties, memory, and kernel-launch helpers) (GPU path; the GPU build is temporarily disabled
 *          in CMake)
 */
